...
# Run database
docker run --name database -v "$PWD/ci-cd/init-db.sh":/docker-entrypoint-initdb.d/init-db.sh -e POSTGRES_PASSWORD=$1 -e POSTGRES_DB=barbershop -p 5432:5432 --net barber-network -d postgres
sleep 3
