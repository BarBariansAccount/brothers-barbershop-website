...
docker run --name barbershop-db -v "$PWD/ci-cd/database/init-db.sh":/docker-entrypoint-initdb.d/init-db.sh -v "$PWD/ci-cd/database/postgres.conf":/etc/postgresql/postgresql.conf -e POSTGRES_PASSWORD=November199853@ -e POSTGRES_DB=barbershop -p 5432:5432 --network barber-network -d postgres:alpine -c 'config_file=/etc/postgresql/postgresql.conf'
